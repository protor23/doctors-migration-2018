...
* Are there any distinguishable patterns? For example:
  + do healthcare workers from a certain region emigrate to a select few other regions?
  + are there any regions to which healthcare workers do not emigrate?

## **Data Origin**

I retrieved data from the [OECD website](https://stats.oecd.org/Index.aspx?DataSetCode=HEALTH_WFMI), where I downloaded migration information from all OECD countries in 2018. 
 
* This refers to the number of healthcare workers (doctors and nurses) who, in 2018, were registered or in the process to gain registration to practise in a country other than the one in which they had obtained their medical education qualifications. 
...
* I chose subregions because they are narrower than regions, but broader than individual countries, hence their number is manageable. This is important to enhance the readability of the data visualization.

I loaded the data in two data frames:
...
You can see the full script [here](https://github.com/protor23/healthcare-workers-migration/blob/main/scripts/01_data-processing.R) for the detailed data processing steps taken. Below I present the major steps. 

I wanted my final dataset to include subregions of origin, subregions of destination, and number of migrants. Therefore, each observation would indicate the number of emigrants from one subregion to another. This required me to join `wf_mig` and `sub_reg` by some variable storing country names, which turned out to difficult.

### **Removing unnecessary data**
I removed:

* flows between identical countries (not interested in domestic migration)
* cases where the number of migrants was 0 (there were only a few, so did not cover)
* unnecessary columns such as the year of migration (since it is only 2018 data) 
* information about regions in sub_reg (since I was only interested in subregions)
* other irrelevant rows (e.g., total migrants per country)

I also replaced column names with suggestive ones to eliminate blanks.
...
I susbequently included keywords of these countries (e.g., Moldova for Republic of Moldova) in a vector(`missing_countries`) and looked for these in `sub_reg` to understand why they were not present there. The code below showed me that the culprit was just a difference in how the countries were named in the two data sets, but the similarities made sense and were easy to check. 
...
```
Therefore, I manually replaced these country names in `sub_reg` to match their `wf_mig` equivalent. 

### **Joining data frames**

I then allocated subregions to each country by joining `wf_mig` and `sub_reg` by country name and provided helpful names to distinguish between origin and destination subregions.
...
_Side note:_ it was surprising to see that people disagree over whether Central Europe is a thing, or whether we should just split Europe into Western, Eastern, Southern, Northern. This data set follows the latter logic.
...
I started by computing the total number of migrants per subregion of origin and destination, irrespective of country, and stred that in the `subregions` data frame:
...
head(subregions)
```
I subsequently created a flow matrix with all subregions as rows and columns which contained only 0s. I treated rows as origin subregions and columns as destination subregions. I updated the values in the `flow_matrix` with the ones in `subregions`.
...
I then stored two versions of these colours, one of which had increased transparency. To accomplish this, I had to split colour codes into individual variables, and then use the `rgb` function to increase transparency.
...
The plot is quite complex and not immediately intuitive, but once the eye is trained, such plot becomes a really nice way of condensing a lot of relevant information in a visually interesting, colourful, neat way. 
...
I started by setting some plotting parameters related to the size of the circle, padding, sector gaps, and the starting plotting point. Please note these are different from the code in the script, as I have changed them to allow some readability on github (I am not sure why the picture looks less crisp on github than when saved as a pdf).
...
I then initialized the layout to allocate subregions into sectors whose sizes are bounded by `xmin` and the `total` flow of migrants for that subregion. This approach ensures the relative size of sectors matches the relative migration flows for each subregion. 
...
### **Final plot** 
Next, I included links between origin subregions and destination subregions, in order to show migration patterns, and an axis to give an indication of actual migrant numbers. This required some further data processing to transform the flow_matrix into its long format and add parameters to guide the position of links, `sums1` and `sums2`. This is explained in depth in the full script. 
...
The colour of links suggests their origin. Notice how immigration patterns are plotted through links starting in the emigrants section of the sector (the second coloured arc inwards), and end up in the immigrants section of another sector (the white arc in continuation of the emigrant arc).

![](figs/01_migration-flow.png)

As we can see, Western Europe has the most migrants, closely followed by Norther Europe and Eastern Europe. However, whilst Western and Northern Europe have relatively low proportions of emigrants and high proportions of immigrants, this is completely the opposite in Eastern Europe. 

Notice that Northern America and Western Europe are the most common immigration destinations, and most individuals in Latin America and the Caribbean emigrate in the same region. Also, very few healthcare workers emigrate to Eastern Europe. 

## **Summary**

Through this project, I have learnt that... working with geographical data sets without proper unique keys is a nightmare when it comes to joins.

On a serious note, I love circular plots - they are not necessarily immediately intuitive (which might cause them to be dismissed at times), but once the eye gets trained into how to interpret them, they become useful because they pack a lot of information. 

On a philosophical note, I wonder what makes a better visualization:

* one that is straightforward to interpret, but relatively common and with limited ability to illustrate complex phenomena such as migration
...
I feel like we're giving too much credit to the former at the expense of the latter, at least in behavioural science. 

However, the danger with this is that one must be careful not to overcomplicate or overcrowd the plot with meaningless details. Fine balance to strike, but I am hoping I can see this type of plot used in behavioural research. 

If I had more time to spend on this project:

* I would attempt to fix the issue that causes some links to go ever so slightly beyond the borders for emigrants - yes, Northern Africa, I'm looking at you! I am still not sure what the reason for this is, but the `circlize` package still receives updates, so I will only keep an eye on it. 
...
* I would evaluate how migration flow patterns change over time, and perhaps visualise this with an interactive plot.
...
* random fantastic stranger on stackoverflow - I am gradeful to you for helping me fix my code errors and helping me learn new ways of doing things